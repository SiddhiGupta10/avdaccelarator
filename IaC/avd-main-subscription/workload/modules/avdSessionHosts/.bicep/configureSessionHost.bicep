...
@sys.description('Storage account name for scripts location.')
param hostStorageAccount string
...
var varBaseScriptArguments = '-ExtendOsDisk ${extendOsDisk} -IdentityServiceProvider ${identityServiceProvider} -Fslogix ${fslogix} -HostPoolRegistrationToken "${hostPool.listRegistrationTokens().value[0].token}" -AmdVmSize ${varAmdVmSize} -NvidiaVmSize ${varNvidiaVmSize} -StorageAccountName ${hostStorageAccount}'
...
resource sessionHostConfig 'Microsoft.Compute/virtualMachines/extensions@2023-09-01' = {
  ...
  properties: {
    ...
    protectedSettings: {
      commandToExecute: 'powershell -ExecutionPolicy Unrestricted -Command .\\${scriptName} ${varScriptArguments} ${environment}'
    }
  }
}

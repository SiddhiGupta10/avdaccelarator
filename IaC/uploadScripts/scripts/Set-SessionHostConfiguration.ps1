...
function Set-FixedPagefileSize {
    ...
    try {
        ...
        if ($current -match '^[A-Z]:\\pagefile\.sys') {
            ...
            Write-Log -Message "Pagefile fixed at $SizeMB MB on $drive" -Category 'Info'
        }
        else {
...
try {
        ...
        if ($Fslogix -eq 'false') {        
                ...
                "C:\Program Files (x86)\Epic\Hyperdrive",
                "C:\Users\*\AppData\Local\Hyperdrive",
                "C:\ProgramData\HyperdriveTempData",
                "C:\Users\*\AppData\Roaming\Hyperdrive"
                )
<# 
...
#>
                ...
                Write-Log -Message 'Enabled Defender exlusions for FSLogix + EPIC paths' -Category 'Info'

                $Processes = @(
                        "%ProgramFiles%\FSLogix\Apps\frxccd.exe",
                        "%ProgramFiles%\FSLogix\Apps\frxccds.exe",
                        "%ProgramFiles%\FSLogix\Apps\frxsvc.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\Core\win-x86\EpicDumpTruckInjector.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\Core\win-x86\DumpTruck\EpicDumpTruckInjector64.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Hyperdrive\Hyperdrive.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\VersionIndependent\Hyperspace.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\VersionIndependent\Launcher.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\EpicPDFSpooler.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\HubFramework.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\Core\win-x86\HubCore.exe",
                        "C:\Program Files (x86)\Epic\Hyperdrive\*\Bin\Core\win-x86\HubSpoke.exe"
                        )

                foreach ($Process in $Processes) {
                        Add-MpPreference -ExclusionProcess $Process
                }
                Write-Log -Message 'Enabled Defender exlusions for FSLogix + EPIC processes' -Category 'Info'
        }
        ...
        try {
                ...
                Write-Log -Message "Set UI Language to fi-FI" -Category 'Info'
                Set-WinUserLanguageList fi-FI -Force
                Write-Log -Message "Set User Language List to fi-FI" -Category 'Info'
                Set-WinSystemLocale fi-FI
                Write-Log -Message "Set System Locale to fi-FI" -Category 'Info'
                Set-Culture fi-FI
                Write-Log -Message "Set Culture to fi-FI" -Category 'Info'
                Set-WinHomeLocation -GeoId 77
                Write-Log -Message "Set Home Location to Finland" -Category 'Info'
                Copy-UserInternationalSettingsToSystem -WelcomeScreen $False -NewUser $True
                Write-Log -Message "Copied international settings to system and new users" -Category 'Info'
        }
        catch {
        ...
        ##############################################################
        #  OneDrive Task Cleanup
        ##############################################################

        Write-Log -Message "=== Starting OneDrive task cleanup at root ===" -Category 'Info'

        # Enumerate all tasks at root with OneDrive in name
        $tasks = Get-ScheduledTask | Where-Object {
        $_.TaskPath -eq "\" -and $_.TaskName -like "*OneDrive*"
        }

        foreach ($task in $tasks) {
        try {
                Write-Log -Message "Removing task: $($task.TaskName)" -Category 'Info'
                Unregister-ScheduledTask -TaskName $task.TaskName -TaskPath "\" -Confirm:$false
                } catch {
                        Write-Log -Message "Failed to remove task: $($task.TaskName)" -Category 'Error'
                }
        }

        Write-Log -Message "=== OneDrive root task cleanup complete ===" -Category 'Info'
...
<#         $services = @("edgeupdate","edgeupdatem")
        ...
        foreach ($svc in $services) {
                try {
                        Write-Log -Message "Stopping service: $svc" -Category 'Info'
                        Stop-Service -Name $svc -Force

                        Write-Log -Message "Disabling service: $svc" -Category 'Info'
                        Set-Service -Name $svc -StartupType Disabled
                }
                catch {
                        Write-Log -Message "Failed to stop/disable service: $svc — $($_.Exception.Message)" -Category 'Error'
                }
        } #>

        # -------------------------------------------------------
        # Windows 11 24H2 Security Baseline. Not appliable to AVD
        # -------------------------------------------------------
        
<#         try {
                # Download the script
                Invoke-WebRequest -Uri "https://stavdshdtstsec007.blob.core.windows.net/scripts/W1124H2SecurityBaseline.zip" `
                                -OutFile "C:\AIB\W1124H2SecurityBaseline.zip"
                
                Expand-Archive -Path "C:\AIB\W1124H2SecurityBaseline.zip" -DestinationPath "C:\AIB\W1124H2SecurityBaseline" -Force
                                
                # Invoke the script with parameters
                . "C:\AIB\W1124H2SecurityBaseline\Scripts\Baseline-LocalInstall.ps1" -Win11NonDomainJoined
                Write-Log -Message "Applied Windows 11 24H2 hardening baseline" -Category 'Info'
        }
        catch {
                # Log the error without propagating it
                Write-Log -Category 'Error' -Message "Windows 11 24H2 hardening baseline failed: $($_.Exception.Message)"
        } #>
        ...
        $edgeSettings = @(
        @{ Name = 'AutoUpdateCheckPeriodMinutes';        PropertyType = 'DWord'; Value = 0 },
        ...
        @{ Name = 'RestoreOnStartup';                    PropertyType = 'DWord'; Value = 4 },
        #@{ Name = 'DeveloperToolsAvailability';          PropertyType = 'DWord'; Value = 2 },
        @{ Name = 'NewTabPageLocation';                  PropertyType = 'String'; Value = 'about:blank' }
        ...
        try {
                $woTarget = 'C:\AIB\windowsOptimization.ps1'
                if (Test-Path $woTarget) {
                        try {
                                Remove-Item -Path $woTarget -Force -ErrorAction Stop
                                Write-Log -Category 'Info' -Message 'Removed existing windowsOptimization.ps1 before download.'
                        }
                        catch {
                                Write-Log -Category 'Warning' -Message "Failed to remove existing windowsOptimization.ps1 (continuing): $($_.Exception.Message)"
                        }
                }
                # Download the latest script
                Invoke-WebRequest -Uri "https://$StorageAccountName.blob.core.windows.net/scripts/windowsOptimization.ps1" -OutFile $woTarget -UseBasicParsing -ErrorAction Stop
                Write-Log -Category 'Info' -Message 'Downloaded latest windowsOptimization.ps1.'

                # Invoke with required optimizations
                . $woTarget -Optimizations 'DefaultUserSettings','Services','ScheduledTasks','AutoLoggers'
                Write-Log -Category 'Info' -Message 'Executed windowsOptimization.ps1 successfully.'
        }
        catch {
        ...
        foreach ($fullPath in $tasks) {
                try {
                        ...
                        if ($taskFolder -ne "\") { $taskFolder += "\" }

                        $task = Get-ScheduledTask -TaskPath $taskFolder -TaskName $taskName

                        # Create a new daily trigger at 03:00
                        $newTrigger = New-ScheduledTaskTrigger -Daily -At 03:00
                        # Only set RandomDelay if supported
                        if ($newTrigger.PSObject.Properties.Name -contains 'RandomDelay') {
                        $newTrigger.RandomDelay = "PT0S"
                        }

                        Set-ScheduledTask -TaskPath $taskFolder -TaskName $taskName -Trigger $newTrigger -Action $task.Actions
                        #Write-Host "Updated $taskName to run daily at 03:00 with no random delay."
                        Write-Log -Message "Updated $taskName to run daily at 03:00 with no random delay." -Category 'Info'
                }
                catch {
                        #Write-Warning "Task not found or could not be updated: $fullPath"
                        Write-Log -Message "Task not found or could not be updated: $fullPath" -Category 'Warning'
                }
        }
...
<#      ##############################################################
        ...
        # Disable per-user Edge and OneDrive scheduled tasks
        $patterns = @(
        @{Path="\"; Pattern="MicrosoftEdgeUpdateTaskMachineCore*"},
        @{Path="\"; Pattern="MicrosoftEdgeUpdateTaskMachineUA*"},
        @{Path="\"; Pattern="OneDrive Reporting Task*"},
        @{Path="\"; Pattern="OneDrive Standalone Update Task*"},
        @{Path="\"; Pattern="OneDrive Startup Task*"}
        )

        foreach ($p in $patterns) {
        Get-ScheduledTask -TaskPath $p.Path -ErrorAction SilentlyContinue |
                Where-Object { $_.TaskName -like $p.Pattern } |
                ForEach-Object {
                Disable-ScheduledTask -TaskName $_.TaskName -TaskPath $_.TaskPath -Confirm:$false
                Write-Log -Message "Disabled task: $($_.TaskPath)$($_.TaskName)" -Category 'Info'
                }
        } #>
        ...
        $fixes = @(
        [PSCustomObject]@{ Issue = 27; Description = "Epic Hyperdrive Config"; Source = "C:\AIB\software\Hyperdrive\Epic Hyperdrive Setup 100.2508.1\491Config.json"; Destination = "C:\Program Files (x86)\Epic\Hyperdrive\Config" },
        ...
        [PSCustomObject]@{ Issue = 1; Description = "Hyperdrive bat script"; Source = "C:\AIB\software\LastConfigurations\Hyperdrive"; Destination = "C:\Sovellukset\Hyperdrive" },
        [PSCustomObject]@{ Issue = 36; Description = "Hyperspace icons"; Source = "C:\AIB\software\LastConfigurations\Hyperspace"; Destination = "C:\Sovellukset\Hyperspace" },
        [PSCustomObject]@{ Issue = 1; Description = "Edge-Apotti-tukiportaali"; Source = "C:\AIB\software\LastConfigurations\tukiportaali"; Destination = "C:\Sovellukset\tukiportaali" },
        ...
        #  Restart VM
        ##############################################################
        Start-Process -FilePath 'shutdown' -ArgumentList '/r /t 60'
        exit 0
}
catch {

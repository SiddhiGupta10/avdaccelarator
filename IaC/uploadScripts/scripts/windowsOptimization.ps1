...
Begin {
        ...
        try {
            ...
            write-Log -Message "AVD AIB Customization : Windows Optimizations " -Category 'Info'

            $WindowsVersion = (Get-ItemProperty "HKLM:\Software\Microsoft\Windows NT\CurrentVersion\").ReleaseId
            $WorkingLocation = (Join-Path $PSScriptRoot $WindowsVersion)
            #$templateFilePathFolder = "C:\AIB"
            ...
            Write-Log -Message "AVD AIB Customization : Windows Optimizations - Windows version is $WindowsVersion" -Category 'Info'
        } 
        catch
        {
            #Write-Host 'AVD AIB Customization : Windows Optimizations - Invalid Path $WorkingLocation - Exiting Script!'
            Write-Log -Message 'AVD AIB Customization : Windows Optimizations - Invalid Path $WorkingLocation - Exiting Script!' -Category 'Error'
            Return
        }
}
PROCESS {
    ...
    if (-not ($PSBoundParameters.Keys -match 'Optimizations') )
     {
        ...
        Write-Log -Message "AVD AIB Customization : Windows Optimizations - No Optimizations (Optimizations or AdvancedOptimizations) passed, exiting script!" -Category 'Error'
        Return
    }
    ...
    If ($Optimizations -contains "DefaultUserSettings" -or $Optimizations -contains "All")
    {
        ...
        If (Test-Path $DefaultUserSettingsFilePath)
        {
            ...
            Write-Log -Message "AVD AIB Customization : Windows Optimizations - - Set Default User Settings" -Category 'Info'
            ...
            If ($UserSettings.Count -gt 0)
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations - Processing Default User Settings (Registry Keys)" -Category 'Info'
                ...
                Foreach ($Item in $UserSettings)
                {
                    ...
                    If (Test-Path -Path ("{0}" -f $Item.HivePath))
                    {
                        ...
                        Write-Log -Message "AVD AIB Customization : Windows Optimizations - Found $($Item.HivePath) - $($Item.KeyName)" -Category 'Info'
                        ...
                        If (Get-ItemProperty -Path ("{0}" -f $Item.HivePath) -ErrorAction SilentlyContinue)
                        {
                            ...
                            Write-Log -Message "AVD AIB Customization : Windows Optimizations - Set $($Item.HivePath) - $Value" -Category 'Info'
                        ...
                        Else
                        {
                            ...
                            Write-Log -Message "AVD AIB Customization : Windows Optimizations- New $($Item.HivePath) Name $($Item.KeyName) PropertyType $($Item.PropertyType) Value $Value" -Category 'Info'
                    ...
                    Else
                    {
                        ...
                        Write-Log -Message "AVD AIB Customization : Windows Optimizations- Registry Path not found $($Item.HivePath)" -Category 'Warning'
                        #Write-Host "AVD AIB Customization : Windows Optimizations- Creating new Registry Key $($Item.HivePath)"
                        Write-Log -Message "AVD AIB Customization : Windows Optimizations- Creating new Registry Key $($Item.HivePath)" -Category 'Info'
                        ...
                        Else
                        {
                            ...
                            Write-Log -Message "AVD AIB Customization : Windows Optimizations- Failed to create new Registry Key" -Category 'Error'
                        } 
                    }
                }
            ...
            Else
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations- No Default User Settings to set" -Category 'Info'
            }
        }
        Else
        {
            #Write-Host "AVD AIB Customization : Windows Optimizations- File not found: $DefaultUserSettingsFilePath"
            Write-Log -Message "AVD AIB Customization : Windows Optimizations- File not found: $DefaultUserSettingsFilePath" -Category 'Error'
        }    
    }
    #endregion
    ...
    If ($Optimizations -contains "AutoLoggers" -or $Optimizations -contains "All")
    {
        ...
        If (Test-Path $AutoLoggersFilePath)
        {
            ...
            Write-Log -Message "AVD AIB Customization : Windows Optimizations- Disable AutoLoggers" -Category 'Info'
            ...
            If ($DisableAutologgers.count -gt 0)
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations- Disable AutoLoggers, Processing Autologger configuration file" -Category 'Info'
                Foreach ($Item in $DisableAutologgers)
                {
                    ...
                    Write-Log -Message "AVD AIB Customization : Windows Optimizations- Updating Registry Key for: $($Item.KeyName)" -Category 'Info'

                    Try 
                    {
                        New-ItemProperty -Path ("{0}" -f $Item.KeyName) -Name "Start" -PropertyType "DWORD" -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
                    }
                    Catch
                    {
                        #Write-Host "AVD AIB Customization : Windows Optimizations- Failed to add $($Item.KeyName)`n`n $($Error[0].Exception.Message)"
                        Write-Log -Message "AVD AIB Customization : Windows Optimizations- Failed to add $($Item.KeyName)`n`n $($Error[0].Exception.Message)" -Category 'Error'
                    }
            ...
            Else 
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations- No Autologgers found to disable" -Category 'Info'
            }
        }
        Else
        {
    ...
    If ($Optimizations -contains "Services" -or $Optimizations -contains "All")
    {
        ...
        If (Test-Path $ServicesFilePath)
        {
            ...
            Write-Log -Message "AVD AIB Customization : Windows Optimizations- Disable Services" -Category 'Info'
            ...
            If ($ServicesToDisable.count -gt 0)
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations- Processing Services Configuration File" -Category 'Info'
                ...
                Foreach ($Item in $ServicesToDisable)
                {
                    ...
                    try {
                        ...
                        $svcConfig = Get-CimInstance Win32_Service -Filter "Name='$($Item.Name)'"
                        write-log -Message ("Service '$($svcConfig.Name)' startup type is now: $($svcConfig.StartMode)") -Category 'Info'
                    }
                    catch {
                        Write-Log -Message "AVD AIB Customization : Windows Optimizations- Failed to disable Service: $($Item.Name) `n $($_.Exception.Message)" -Category 'Error'
                    }
            ...
            Else
            {
                ...
                Write-Log -Message "AVD AIB Customization : Windows Optimizations- No Services found to disable" -Category 'Info'
            }
        }
        Else
        {
            #Write-Host "AVD AIB Customization : Windows Optimizations- File not found: $ServicesFilePath" 
            Write-Log -Message "AVD AIB Customization : Windows Optimizations- File not found: $ServicesFilePath" -Category 'Error'
...
END {
    ...
    <# if ((Test-Path -Path $templateFilePathFolder -ErrorAction SilentlyContinue)) {
        Remove-Item -Path $templateFilePathFolder -Force -Recurse -ErrorAction SilentlyContinue
    }

    if ((Test-Path -Path $WorkingLocation -ErrorAction SilentlyContinue)) {
        Remove-Item -Path $WorkingLocation -Force -Recurse -ErrorAction SilentlyContinue
    } #>
    
    $stopwatch.Stop()
    $elapsedTime = $stopwatch.Elapsed
    #Write-Host "*** AVD AIB CUSTOMIZER PHASE : Windows Optimizations - Exit Code: $LASTEXITCODE ***"
    Write-Log -Message "AVD AIB Customization : Windows Optimizations - Exit Code: $LASTEXITCODE ***" -Category 'Info'    
    ...
    Write-Log -Message "AVD AIB Customization : Windows Optimizations - Ending AVD AIB Customization : Windows Optimizations - Time taken: $elapsedTime" -Category 'Info'
}